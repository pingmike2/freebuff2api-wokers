...
function renderDonut(byModel){var el=$('modelShare');if(!byModel||!Object.keys(byModel).length){el.innerHTML='<p class="muted">No model usage yet.</p>';return}var total=0;var entries=Object.keys(byModel).map(function(m){total+=byModel[m].requests||0;return{m:m,req:byModel[m].requests||0}}).sort(function(a,b){return b.req-a.req});if(!total){el.innerHTML='<p class="muted">No model usage yet.</p>';return}var r=52,c=2*Math.PI*r;var off=0;var segs=entries.map(function(e,i){var frac=e.req/total;var seg='<circle r="'+r+'" cx="70" cy="70" fill="none" stroke="'+DONUT_COLORS[i%DONUT_COLORS.length]+'" stroke-width="16" stroke-dasharray="'+(frac*c).toFixed(1)+' '+(c-frac*c).toFixed(1)+'" stroke-dashoffset="'+(-off*c).toFixed(1)+'" transform="rotate(-90 70 70)"><title>'+esc(e.m)+': '+e.req+' ('+Math.round(frac*100)+'%)</title></circle>';off+=frac;return seg}).join('');var legend=entries.map(function(e,i){return '<div class="muted" style="display:flex;align-items:center;gap:6px;margin:2px 0"><span style="display:inline-block;width:10px;height:10px;background:'+DONUT_COLORS[i%DONUT_COLORS.length]+';border-radius:1px"></span><code>'+esc(e.m)+'</code><b>'+e.req+'</b><span>('+Math.round(e.req/total*100)+'%)</span></div>'}).join('');el.innerHTML='<div class="row" style="align-items:flex-start"><svg viewBox="0 0 140 140" style="width:150px;height:auto">'+segs+'<text x="70" y="75" text-anchor="middle" font-size="15" fill="var(--text)">'+total+'</text></svg><div style="margin-left:14px;flex:1;min-width:240px">'+legend+'</div></div>'}
