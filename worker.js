...
function drawChart(days,bdm,sel){var el=$('chart');if(!days||!days.length){el.innerHTML='<p class="muted">No daily data yet.</p>';return}var data=days.map(function(d){if(sel&&sel!=='all'&&bdm&&bdm[d.date]){var m=bdm[d.date][sel]||{};return{req:(m.requests||0),ok:(m.successes||0),err:(m.errors||0)}}var ok=d.successes||0,err=d.errors||0;return{req:ok+err,ok:ok,err:err}});var max=1;data.forEach(function(v){if(v.req>max)max=v.req});var w=560,h=120,pad=4,bw=Math.max(4,(w-pad*2)/Math.max(data.length,1)-2);var bars=data.map(function(v,i){var x=pad+i*(bw+2);var eh=v.req?Math.max(1,Math.round((v.err/max)*(h-24))):0;var oh=v.req?Math.max(1,Math.round((v.ok/max)*(h-24))):0;var yTop=h-20-eh-oh;var tt=days[i].date+': '+v.req+' req ('+v.ok+' ok, '+v.err+' err)';var lab=v.req?'<text x="'+(x+bw/2)+'" y="'+(yTop-3)+'" font-size="10" text-anchor="middle" fill="var(--muted)">'+v.req+'</text>':'';var fr='';if(v.err)fr+='<rect x="'+x+'" y="'+(h-20-eh)+'" width="'+bw+'" height="'+eh+'" rx="1" fill="var(--err)" style="animation-delay:'+(i*25)+'ms"><title>'+tt+'</title></rect>';if(v.ok)fr+='<rect x="'+x+'" y="'+yTop+'" width="'+bw+'" height="'+oh+'" rx="1" fill="var(--accent)" style="animation-delay:'+(i*25)+'ms"><title>'+tt+'</title></rect>';return fr+lab}).join('');var base='<line x1="'+pad+'" y1="'+(h-20)+'" x2="'+(w-pad)+'" y2="'+(h-20)+'" stroke="var(--border)" stroke-width="1"/>';var labels=days.map(function(d,i){var x=pad+i*(bw+2);return '<text x="'+x+'" y="'+(h-6)+'" font-size="11" fill="var(--muted)">'+d.date.slice(5)+'</text>'}).join('');var legend='<div class="row" style="margin-bottom:6px"><span class="muted" style="display:inline-flex;align-items:center;gap:4px"><span style="display:inline-block;width:10px;height:10px;background:var(--accent);border-radius:1px"></span>ok</span><span class="muted" style="display:inline-flex;align-items:center;gap:4px;margin-left:12px"><span style="display:inline-block;width:10px;height:10px;background:var(--err);border-radius:1px"></span>err</span></div>';el.innerHTML=legend+'<svg viewBox="0 0 '+w+' '+h+'" style="width:100%;height:auto">'+base+bars+labels+'</svg>'}
